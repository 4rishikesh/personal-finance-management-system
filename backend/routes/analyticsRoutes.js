...
router.get("/highest-category", authMiddleware, async (req, res) => {
  try {
    ...
    for (const cat in categoryData) {
      if (categoryData[cat] > highestAmount) {
        highestAmount = categoryData[cat];
        highestCategory = cat;
      }
    }

    res.json({ category: highestCategory, amount: highestAmount });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.get("/runway", authMiddleware, async (req, res) => {
  try {
    ...
    const days = Math.max(1, Math.ceil((new Date() - oldest) / (1000 * 60 * 60 * 24)));
    const averageDailyExpense = expense / days;
    const runwayDays = balance > 0 && averageDailyExpense > 0
      ? balance / averageDailyExpense
      : 0;

    let riskLevel;
    if (runwayDays === 0) riskLevel = "Critical";
    else if (runwayDays < 30) riskLevel = "High";
    else if (runwayDays < 60) riskLevel = "Moderate";
    else riskLevel = "Low";
